...
func TestByzantinePrevoteEquivocation(t *testing.T) {
	...
	lazyProposer.decideProposal = func(height int64, round int32) {
		...
		p := proposal.ToProto()
		if err := lazyProposer.privValidator.SignProposal(lazyProposer.state.ChainID, lazyProposer.state.Validators.QuorumHash, p); err == nil {
...
func byzantineDecideProposalFunc(t *testing.T, height int64, round int32, cs *State, sw *p2p.Switch) {
	...
	if err := cs.privValidator.SignProposal(cs.state.ChainID, cs.Validators.QuorumHash, p1); err != nil {
		t.Error(err)
	}
	...
	if err := cs.privValidator.SignProposal(cs.state.ChainID, cs.Validators.QuorumHash, p2); err != nil {
		t.Error(err)
	}
