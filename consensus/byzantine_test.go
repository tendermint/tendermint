...
func sendProposalAndParts(
	...
	// votes
	cs.mtx.Lock()
	prevote, _ := cs.signVote(types.SIGNED_MSG_TYPE_PREVOTE_TYPE, blockHash, parts.Header())
	precommit, _ := cs.signVote(types.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, blockHash, parts.Header())
	cs.mtx.Unlock()
