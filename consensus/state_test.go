...
func TestStateProposerSelection0(t *testing.T) {
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, rs.ProposalBlock.Hash(), rs.ProposalBlockParts.Header(), vss[1:]...)
...
func TestStateProposerSelection2(t *testing.T) {
	...
	for i := 0; i < len(vss); i++ {
		...
		rs := cs1.GetRoundState()
		signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, nil, rs.ProposalBlockParts.Header(), vss[1:]...)
...
func TestStateBadProposal(t *testing.T) {
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, propBlock.Hash(), propBlock.MakePartSet(partSize).Header(), vs2)
	ensurePrevote(voteCh, height, round)

	// wait for precommit
	ensurePrecommit(voteCh, height, round)
	validatePrecommit(t, cs1, round, -1, vss[0], nil, nil)
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, propBlock.Hash(), propBlock.MakePartSet(partSize).Header(), vs2)
}
...
func TestStateFullRound2(t *testing.T) {
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, propBlockHash, propPartsHeader, vs2)
	ensurePrevote(voteCh, height, round) // prevote
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, propBlockHash, propPartsHeader, vs2)
	ensurePrecommit(voteCh, height, round)
...
func TestStateLockNoPOL(t *testing.T) {
	...
	*/
	...
	// prevote arrives from vs2:
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, theBlockHash, thePartSetHeader, vs2)
	ensurePrevote(voteCh, height, round) // prevote
	...
	hash[0] = (hash[0] + 1) % 255
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, hash, thePartSetHeader, vs2)
	ensurePrecommit(voteCh, height, round) // precommit
	...
	*/
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, hash, rs.LockedBlock.MakePartSet(partSize).Header(), vs2)
	ensurePrevote(voteCh, height, round)
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, hash, rs.LockedBlock.MakePartSet(partSize).Header(), vs2)
	ensurePrecommit(voteCh, height, round)
	...
	*/
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, hash, rs.ProposalBlock.MakePartSet(partSize).Header(), vs2)
	ensurePrevote(voteCh, height, round)
	...
	signAddVotes(
		cs1,
		proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE,
		hash,
...
func TestStateLockPOLRelock(t *testing.T) {
	...
	*/
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, theBlockHash, theBlockParts, vs2, vs3, vs4)
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, nil, types.PartSetHeader{}, vs2, vs4)
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, theBlockHash, theBlockParts, vs3)
	...
	*/
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, propBlockHash, propBlockParts.Header(), vs2, vs3, vs4)

	ensurePrecommit(voteCh, height, round)
	// we should have unlocked and locked on the new block
	validatePrecommit(t, cs1, round, round, vss[0], propBlockHash, propBlockHash)

	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, propBlockHash, propBlockParts.Header(), vs2, vs3)
...
func TestStateLockPOLUnlock(t *testing.T) {
	...
	*/
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, nil, types.PartSetHeader{}, vs2, vs3, vs4)
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, nil, types.PartSetHeader{}, vs2, vs3)
...
func TestStateLockPOLSafety1(t *testing.T) {
	...
	prevotes := signVotes(proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, propBlock.Hash(), propBlock.MakePartSet(partSize).Header(), vs2, vs3, vs4)

	t.Logf("old prop hash %v", fmt.Sprintf("%X", propBlock.Hash()))

	// we do see them precommit nil
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, nil, types.PartSetHeader{}, vs2, vs3, vs4)
...
func TestStateLockPOLSafety2(t *testing.T) {
	...
	// the others sign a polka but we don't see it
	prevotes := signVotes(proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, propBlockHash0, propBlockParts0.Header(), vs2, vs3, vs4)
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, propBlockHash1, propBlockParts1.Header(), vs2, vs3, vs4)
	...
	// add precommits from the rest
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, nil, types.PartSetHeader{}, vs2, vs4)
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, propBlockHash1, propBlockParts1.Header(), vs3)
...
func TestProposeValidBlock(t *testing.T) {
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, propBlockHash, propBlock.MakePartSet(partSize).Header(), vs2, vs3, vs4)
...
func TestSetValidBlockOnDelayedPrevote(t *testing.T) {
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, propBlockHash, propBlockParts.Header(), vs2)

	// vs3 send prevote nil
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, nil, types.PartSetHeader{}, vs3)
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, propBlockHash, propBlockParts.Header(), vs4)
...
func TestEmitNewValidBlockEventOnCommitWithoutBlock(t *testing.T) {
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, propBlockHash, propBlockParts.Header(), vs2, vs3, vs4)
	ensureNewValidBlock(validBlockCh, height, round)
...
func TestStartNextHeightCorrectly(t *testing.T) {
	...
	// add precommits
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, nil, types.PartSetHeader{}, vs2)
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, theBlockHash, theBlockParts, vs3)
	time.Sleep(5 * time.Millisecond)
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, theBlockHash, theBlockParts, vs4)
...
func TestStateSlashingPrevotes(t *testing.T) {
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, hash, rs.ProposalBlockParts.Header(), vs2)
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, rs.ProposalBlock.Hash(), rs.ProposalBlockParts.Header(), vs2)
...
func TestStateSlashingPrecommits(t *testing.T) {
	...
	hash := rs.ProposalBlock.Hash()
	hash[0] = byte(hash[0]+1) % 255
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, hash, rs.ProposalBlockParts.Header(), vs2)

	// NOTE: we have to send the vote for different block first so we don't just go into precommit round right
	// away and ignore more prevotes (and thus fail to slash!)

	// add precommit from vs2
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, rs.ProposalBlock.Hash(), rs.ProposalBlockParts.Header(), vs2)
...
func TestStateHalt1(t *testing.T) {
	...
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PREVOTE_TYPE, propBlock.Hash(), propBlockParts.Header(), vs2, vs3, vs4)
	...
	// add precommits from the rest
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, nil, types.PartSetHeader{}, vs2) // didnt receive proposal
	signAddVotes(cs1, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, propBlock.Hash(), propBlockParts.Header(), vs3)
	// we receive this later, but vs3 might receive it earlier and with ours will go to commit!
	precommit4 := signVote(vs4, proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, propBlock.Hash(), propBlockParts.Header())
...
func TestStateOutputVoteStats(t *testing.T) {
	...
	vote := signVote(vss[1], proto.SIGNED_MSG_TYPE_PRECOMMIT_TYPE, []byte("test"), types.PartSetHeader{})
