...
func TestStateFullRound1(t *testing.T) {
	...
	// grab proposal
	re := <-propCh
	propBlockHash := re.(types.TMEventData).(types.EventDataRoundState).RoundState.(*cstypes.RoundState).ProposalBlock.Hash()
...
func TestStateLockNoPOL(t *testing.T) {
	...
	*/
	...
	re := <-proposalCh
	rs := re.(types.TMEventData).(types.EventDataRoundState).RoundState.(*cstypes.RoundState)
...
func TestStateLockPOLRelock(t *testing.T) {
	...
	*/
	...
	be := <-newBlockCh
	b := be.(types.TMEventData).(types.EventDataNewBlockHeader)
	re = <-newRoundCh
	rs = re.(types.TMEventData).(types.EventDataRoundState).RoundState.(*cstypes.RoundState)
	if rs.Height != 2 {
