...
func (h *Handshaker) replayBlocks(state sm.State, proxyApp proxy.AppConns, appBlockHeight, storeBlockHeight int64, mutateState bool) ([]byte, error) {
	...
	for i := appBlockHeight + 1; i <= finalBlock; i++ {
		...
		//#3460 enhance replayBlocks of checkAppHash
		nextBlock := h.store.LoadBlock(i + 1)
		if nextBlock != nil {
			//Ehach block's apphash should be checked  not only the last block
...
//which block's apphash does't match should be known
