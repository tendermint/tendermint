...
func (cs *State) defaultDoPrevote(height int64, round int) {
	...
	if cs.LockedBlock != nil {
		...
		cs.signAddVote(types.SIGNED_MSG_TYPE_PREVOTE, cs.LockedBlock.Hash(), cs.LockedBlockParts.Header())
		return
	}
	...
	if cs.ProposalBlock == nil {
		...
		cs.signAddVote(types.SIGNED_MSG_TYPE_PREVOTE, nil, types.PartSetHeader{})
		return
	}
	...
	cs.signAddVote(types.SIGNED_MSG_TYPE_PREVOTE, cs.ProposalBlock.Hash(), cs.ProposalBlockParts.Header())
}
...
func (cs *State) enterPrecommit(height int64, round int) {
	...
	if !ok {
		...
		cs.signAddVote(types.SIGNED_MSG_TYPE_PRECOMMIT, nil, types.PartSetHeader{})
		return
	}
	...
	if cs.LockedBlock.HashesTo(blockID.Hash) {
		...
		cs.signAddVote(types.SIGNED_MSG_TYPE_PRECOMMIT, blockID.Hash, blockID.PartsHeader)
		return
	}
...
func (cs *State) addVote(
	...
	if vote.Height+1 == cs.Height {
		if !(cs.Step == cstypes.RoundStepNewHeight && vote.Type == types.SIGNED_MSG_TYPE_PRECOMMIT) {
	...
	switch vote.Type {
	case types.SIGNED_MSG_TYPE_PREVOTE:
	...
	case types.SIGNED_MSG_TYPE_PRECOMMIT:
