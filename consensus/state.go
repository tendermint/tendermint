...
func (cs *State) defaultDoPrevote(height int64, round int) {
	...
	if cs.LockedBlock != nil {
		...
		cs.signAddVote(types.prevote, cs.LockedBlock.Hash(), cs.LockedBlockParts.Header())
		return
	}
	...
	if cs.ProposalBlock == nil {
		...
		cs.signAddVote(types.prevote, nil, types.PartSetHeader{})
		return
	}
	...
	cs.signAddVote(types.prevote, cs.ProposalBlock.Hash(), cs.ProposalBlockParts.Header())
}
...
func (cs *State) enterPrecommit(height int64, round int) {
	...
	if !ok {
		...
		cs.signAddVote(types.precommit, nil, types.PartSetHeader{})
		return
	}
	...
	if cs.LockedBlock.HashesTo(blockID.Hash) {
		...
		cs.signAddVote(types.precommit, blockID.Hash, blockID.PartsHeader)
		return
	}
...
func (cs *State) addVote(
	...
	if vote.Height+1 == cs.Height {
		if !(cs.Step == cstypes.RoundStepNewHeight && vote.Type == types.precommit) {
