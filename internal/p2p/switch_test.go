...
func TestSwitchPeerFilterDuplicate(t *testing.T) {
	sw := MakeSwitch(cfg, 1, "testing", "123.123.123", nil, initSwitchFunc, log.TestingLogger())
	err := sw.Start()
	require.NoError(t, err)
	t.Cleanup(func() {
		if err := sw.Stop(); err != nil {
...
func TestSwitchRejectsIncompatiblePeers(t *testing.T) {
	sw := MakeSwitch(cfg, 1, "127.0.0.1", "123.123.123", nil, initSwitchFunc, log.TestingLogger())
	ni := sw.NodeInfo()
	ni.Network = "network-a"
	sw.SetNodeInfo(ni)
