...
func (s *lightClientStateProvider) AppHash(ctx context.Context, height uint64) ([]byte, error) {
	...
	header, err := s.lc.VerifyLightBlockAtHeight(ctx, height+1, time.Now())
	if err != nil {
		return nil, err
	}
	...
	_, err = s.lc.VerifyLightBlockAtHeight(ctx, height+2, time.Now())
	if err != nil {
		return nil, err
	}
	_, err = s.lc.VerifyLightBlockAtHeight(ctx, height, time.Now())
	if err != nil {
		return nil, err
	}
...
func (s *lightClientStateProvider) Commit(ctx context.Context, height uint64) (*types.Commit, error) {
	s.Lock()
	defer s.Unlock()
	header, err := s.lc.VerifyLightBlockAtHeight(ctx, height, time.Now())
	if err != nil {
		return nil, err
	}
...
func (s *lightClientStateProvider) State(ctx context.Context, height uint64) (sm.State, error) {
	...
	lastLightBlock, err := s.lc.VerifyLightBlockAtHeight(ctx, height, time.Now())
	if err != nil {
		return sm.State{}, err
	}
	currentLightBlock, err := s.lc.VerifyLightBlockAtHeight(ctx, height+1, time.Now())
	if err != nil {
		return sm.State{}, err
	}
	nextLightBlock, err := s.lc.VerifyLightBlockAtHeight(ctx, height+2, time.Now())
	if err != nil {
		return sm.State{}, err
	}
