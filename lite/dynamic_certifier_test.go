...
func TestDynamicCert(t *testing.T) {
	...
	for _, tc := range cases {
		check := tc.keys.GenCommit(chainID, tc.height, nil, tc.vals,
			[]byte("foo"), []byte("bar"), []byte("params"), []byte("results"), tc.first, tc.last)
...
func TestDynamicUpdate(t *testing.T) {
	...
	h := int64(100)
	good := keys.GenCommit(chainID, h, nil, vals, []byte("foo"), []byte("bar"), []byte("params"), []byte("results"), 0, len(keys))
