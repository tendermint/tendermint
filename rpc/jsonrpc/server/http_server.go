...
func Serve(listener net.Listener, handler http.Handler, logger log.Logger, config *Config) error {
	logger.Info("serve", "msg", log.LazySprintf("Starting RPC HTTP server on %s", listener.Addr()))
	s := &http.Server{
...
func ServeTLS(
	...
	logger.Info("serve tls", "msg", log.LazySprintf("Starting RPC HTTPS server on %s (cert: %q, key: %q)",
