...
func NewNode(config *cfg.Config,
	...
	if config.RPC.PprofListenAddress != "" {
		go func() {
			...
			logger.Error("pprof server error", "err", http.ListenAndServe(config.RPC.PprofListenAddress, nil)) //nolint:gosec,lll // we know we're using net/http.Serve with no support for timeouts
		}()
	}
