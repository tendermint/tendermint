...
func (sm *simpleMap) Set(key string, value []byte) {
	...
	sm.kvs = append(sm.kvs, kv.KVPair{
		Key:   []byte(key),
		Value: vhash,
	})
}
...
func (sm *simpleMap) KVPairs() kv.KVPairs {
	sm.Sort()
	kvs := make(kv.KVPairs, len(sm.kvs))
	copy(kvs, sm.kvs)
	return kvs
}
...
func hashKVPairs(kvs kv.KVPairs) []byte {
