...
func RegisterCodec(c *amino.Codec) {
	...
	c.RegisterConcrete(PubKeySr25519{}, PubKeyAminoName, nil)

	c.RegisterInterface((*crypto.PrivKey)(nil), nil)
	c.RegisterConcrete(PrivKeySr25519{}, PrivKeyAminoName, nil)
}

// MarshalBinary attempts to marshal a PrivKeySr25519 type that is backwards
...
func (privKey PrivKeySr25519) MarshalBinary() ([]byte, error) {
...
func (privKey *PrivKeySr25519) UnmarshalBinary(bz []byte) error {
	lbz := []byte{lengthPrivKeySr25519}
	p := len(prefixPrivKeySr25519)
	l := len(lbz)
...
// it. For now, clients must call MarshalBinary directly on the type to get the
// custom compatible encoding.
func (pubKey PubKeySr25519) MarshalBinary() ([]byte, error) {
...
// it. For now, clients must call UnmarshalBinary directly on the type to get the
// custom compatible decoding.
func (pubKey *PubKeySr25519) UnmarshalBinary(bz []byte) error {
	lbz := []byte{lengthPubKeySr25519}
	p := len(prefixPubKeySr25519)
	l := len(lbz)
