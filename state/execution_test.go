...
func TestBeginBlockValidators(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		// block for height 2
		block, _ := state.MakeBlock(2, makeTxs(2), lastCommit, nil, state.Validators.GetProposer().Address,nil)
...
func TestBeginBlockByzantineValidators(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		block, _ := state.MakeBlock(10, makeTxs(2), lastCommit, nil, state.Validators.GetProposer().Address,nil)
		block.Time = now
...
func makeBlock(state State, height int64) *types.Block {
	block, _ := state.MakeBlock(height, makeTxs(state.LastBlockHeight), new(types.Commit), nil, state.Validators.GetProposer().Address,nil)
	return block
}
