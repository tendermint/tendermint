...
func TestBeginBlockValidators(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		_, err = sm.ExecCommitBlock(proxyApp.Consensus(), block, log.TestingLogger(), stateDB, 0)
...
func TestBeginBlockByzantineValidators(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		block, _ := state.MakeBlock(10, makeTxs(2), lastCommit, nil, state.Validators.GetProposer().Address, 0)
		block.Time = now
