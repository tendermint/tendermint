...
func Load(ctx context.Context, testnet *e2e.Testnet, multiplier int) error {
	...
	logger.Info("load", log.LazySprintf("Starting transaction load (%v workers)...", concurrency))
	started := time.Now()
	...
	for {
		...
		case <-ctx.Done():
			...
			logger.Info("load", log.LazySprintf("Ending transaction load after %v txs (%.1f tx/s)...",
