...
func Test_ResetAll(t *testing.T) {
	...
	require.NoFileExists(t, filepath.Join(config.DBDir(), "block0.db"))
	require.NoFileExists(t, filepath.Join(config.DBDir(), "state0.db"))
	require.NoFileExists(t, filepath.Join(config.DBDir(), "evidence0.db"))
	require.NoFileExists(t, filepath.Join(config.DBDir(), "tx_index0.db"))
