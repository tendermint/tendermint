...
* **Usage**:
    ...
    * Tendermint MAY include a list of transactions in `RequestPrepareProposal.txs` whose total size in bytes exceeds `RequestPrepareProposal.max_tx_bytes`.
      Therefore:
        * When setting `ResponsePrepareProposal.modified_tx_status` to `MODIFIED`, the Application MUST always make sure that
          the `RequestPrepareProposal.max_tx_bytes` limit is respected by the list of transaction records it returns
          in `ResponsePrepareProposal.tx_records`, excluding those marked as `REMOVE`.
        * If the size of `RequestPrepareProposal.txs` is greater than `RequestPrepareProposal.max_tx_bytes`, it is _invalid_ to return
          `ResponsePrepareProposal.modified_tx_status` set to `UNMODIFIED`: Tendermint will panic.
