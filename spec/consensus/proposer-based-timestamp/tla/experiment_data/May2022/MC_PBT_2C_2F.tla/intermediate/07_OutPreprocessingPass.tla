...
*)
Init ==
  round = [ p$9 \in { "c1", "c2" } |-> 0 ]
    /\ localClock \in [{ "c1", "c2" } -> (2 .. 2 + 2)]
    /\ realTime = 0
    /\ step = [ p$10 \in { "c1", "c2" } |-> "PROPOSE" ]
    /\ decision
      = [ p$11 \in { "c1", "c2" } |-> <<<<"None", (-1), (-1)>>, (-1)>> ]
    /\ lockedValue = [ p$12 \in { "c1", "c2" } |-> "None" ]
    /\ lockedRound = [ p$13 \in { "c1", "c2" } |-> -1 ]
    /\ validValue = [ p$14 \in { "c1", "c2" } |-> <<"None", (-1), (-1)>> ]
    /\ validRound = [ p$15 \in { "c1", "c2" } |-> -1 ]
    /\ ((TRUE
    ...
    /\ proposalReceptionTime
      = [
        r_p$1 \in
          { <<t_r$1, t_s$1>>: t_r$1 \in 0 .. 3, t_s$1 \in { "c1", "c2" } } |->
          -1
      ]
    /\ evidence = {}
    /\ action = "Init"
    /\ beginRound
      = [
        r_c$1 \in
          { <<t_t$1, t_u$1>>: t_t$1 \in 0 .. 3, t_u$1 \in { "c1", "c2" } } |->
  ...
  (realTime < 7
      /\ (\E t$4 \in 0 .. 7:
        t$4 > realTime
          /\ realTime' = t$4
          /\ localClock'
            = [ p$16 \in { "c1", "c2" } |-> localClock[p$16] + t$4 - realTime ])
      /\ ((round' = round
          /\ step' = step
          /\ decision' = decision
...
InitPrimed ==
  round' = [ p$19 \in { "c1", "c2" } |-> 0 ]
    /\ (\E t_1o$1 \in [{ "c1", "c2" } -> (2 .. 2 + 2)]: localClock' = t_1o$1)
    /\ realTime' = 0
    /\ step' = [ p$20 \in { "c1", "c2" } |-> "PROPOSE" ]
    /\ decision'
      = [ p$21 \in { "c1", "c2" } |-> <<<<"None", (-1), (-1)>>, (-1)>> ]
    /\ lockedValue' = [ p$22 \in { "c1", "c2" } |-> "None" ]
    /\ lockedRound' = [ p$23 \in { "c1", "c2" } |-> -1 ]
    /\ validValue' = [ p$24 \in { "c1", "c2" } |-> <<"None", (-1), (-1)>> ]
    /\ validRound' = [ p$25 \in { "c1", "c2" } |-> -1 ]
    /\ ((TRUE
