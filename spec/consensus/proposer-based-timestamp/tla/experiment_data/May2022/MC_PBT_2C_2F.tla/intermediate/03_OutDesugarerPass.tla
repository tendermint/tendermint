...
(*
  @type: (() => Bool);
*)
CorrectProposer == Proposer \in [(Rounds) -> { "c1", "c2" }]
  ...
  ValidTime(realTime)
    /\ (\E t \in Timestamps:
      t > realTime
        /\ realTime' = t
        /\ localClock'
          = [ p \in { "c1", "c2" } |-> localClock[p] + t - realTime ])
    /\ (coreVars' = coreVars
...
*)
FaultyPrevotes(r) ==
  [type: {"PREVOTE"}, src: { "f3", "f4" }, round: {r}, id: Proposals]

(*
  @type: (() => (Int -> Int));
*)
lastBeginRound ==
  [ r \in Rounds |-> Max({ beginRound[r, p]: p \in { "c1", "c2" } }) ]
...
*)
firstBeginRound ==
  [ r \in Rounds |-> Min({ beginRound[r, p]: p \in { "c1", "c2" } }) ]
...
*)
AllFaultyPrecommits ==
  [type: {"PRECOMMIT"}, src: { "f3", "f4" }, round: Rounds, id: Proposals]
...
*)
AllFaultyPrevotes ==
  [type: {"PREVOTE"}, src: { "f3", "f4" }, round: Rounds, id: Proposals]
...
*)
FaultyPrecommits(r) ==
  [type: {"PRECOMMIT"}, src: { "f3", "f4" }, round: {r}, id: Proposals]
...
Init ==
  round = [ p \in { "c1", "c2" } |-> 0 ]
    ...
    /\ realTime = 0
    /\ step = [ p \in { "c1", "c2" } |-> "PROPOSE" ]
    /\ decision = [ p \in { "c1", "c2" } |-> NilDecision ]
    /\ lockedValue = [ p \in { "c1", "c2" } |-> NilValue ]
    /\ lockedRound = [ p \in { "c1", "c2" } |-> NilRound ]
    /\ validValue = [ p \in { "c1", "c2" } |-> NilProposal ]
    /\ validRound = [ p \in { "c1", "c2" } |-> NilRound ]
    /\ InitMsgs
    /\ proposalReceptionTime
      = [ r_p \in Rounds \X { "c1", "c2" } |-> NilTimestamp ]
    /\ evidence = {}
    /\ action = "Init"
    /\ beginRound
      = [
        r_c \in Rounds \X { "c1", "c2" } |->
...
Next ==
  AdvanceRealTime
    \/ FaultyBroadcast
    \/ (SynchronizedLocalClocks
      /\ (\E p \in { "c1", "c2" }: MessageProcessing(p)))
