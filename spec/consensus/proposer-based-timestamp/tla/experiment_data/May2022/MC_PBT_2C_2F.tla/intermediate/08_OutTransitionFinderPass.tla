...
*)
CInit_si_0 ==
  \E t_1n$1 \in [(0 .. 3) -> ({ "c1", "c2" } \union { "f3", "f4" })]:
    Proposer' := t_1n$1
...
Init_si_0000 ==
  round' := [ p$1 \in { "c1", "c2" } |-> 0 ]
    /\ (\E t_1o$1 \in [{ "c1", "c2" } -> (2 .. 2 + 2)]: localClock' := t_1o$1)
    /\ realTime' := 0
    /\ step' := [ p$2 \in { "c1", "c2" } |-> "PROPOSE" ]
    /\ decision'
      := [ p$3 \in { "c1", "c2" } |-> <<<<"None", (-1), (-1)>>, (-1)>> ]
...
*)
Init_si_0001 ==
  round' := [ p$8 \in { "c1", "c2" } |-> 0 ]
    /\ (\E t_1o$2 \in [{ "c1", "c2" } -> (2 .. 2 + 2)]: localClock' := t_1o$2)
    /\ realTime' := 0
    /\ step' := [ p$9 \in { "c1", "c2" } |-> "PROPOSE" ]
    /\ decision'
      := [ p$10 \in { "c1", "c2" } |-> <<<<"None", (-1), (-1)>>, (-1)>> ]
    /\ lockedValue' := [ p$11 \in { "c1", "c2" } |-> "None" ]
    /\ lockedRound' := [ p$12 \in { "c1", "c2" } |-> -1 ]
    /\ validValue' := [ p$13 \in { "c1", "c2" } |-> <<"None", (-1), (-1)>> ]
    /\ validRound' := [ p$14 \in { "c1", "c2" } |-> -1 ]
    /\ (FALSE
    ...
    /\ proposalReceptionTime'
      := [
        r_p$2 \in
          { <<t_2h$2, t_2i$2>>: t_2h$2 \in 0 .. 3, t_2i$2 \in { "c1", "c2" } } |->
          -1
      ]
    /\ evidence' := {}
    /\ action' := "Init"
    /\ beginRound'
      := [
        r_c$2 \in
          { <<t_2j$2, t_2k$2>>: t_2j$2 \in 0 .. 3, t_2k$2 \in { "c1", "c2" } } |->
        ...
        msgsPrecommit'
            := [
              msgsPrecommit EXCEPT
                ![r$7] = msgsPrecommit[r$7] \union msgs$1
            ]
          /\ ((round' := round
              /\ step' := step
              /\ decision' := decision
...
Next_si_0002 ==
  (\A p$16 \in { "c1", "c2" }:
    ...
    /\ (\E p$17 \in { "c1", "c2" }:
      (\E MyEvidence$1 \in SUBSET (msgsPrecommit[round[p$17]]):
          LET (*
            @type: (() => Set(Str));
          *)
                ...
                *)
                ...
                msgsPrevote'
                    := [
                      msgsPrevote EXCEPT
                        ![r_si_12] =
                          msgsPrevote[(r_si_12)] \union {(newMsg_si_2)}
                    ]
      ...
      \E r$13 \in 0 .. 3:
        r$13 > round[p$29]
          ...
          *)
          ...
          IN
          \E MyEvidence$2 \in SUBSET RoundMsgs_si_1:
            LET (*
              @type: (() => Set(Str));
            *)
  ...
  FALSE
    ...
    /\ (\E r$15 \in 0 .. 3:
      \E msgs$2 \in SUBSET ({
        ...
        msgsPrevote'
            := [ msgsPrevote EXCEPT ![r$15] = msgsPrevote[r$15] \union msgs$2 ]
          /\ ((round' := round
              /\ step' := step
              /\ decision' := decision
              /\ lockedValue' := lockedValue
  ...
  FALSE
    ...
    /\ (\E r$17 \in 0 .. 3:
      \E msgs$3 \in SUBSET ({
        ...
        msgsPropose'
            := [ msgsPropose EXCEPT ![r$17] = msgsPropose[r$17] \union msgs$3 ]
          /\ ((round' := round
              /\ step' := step
              /\ decision' := decision
              /\ lockedValue' := lockedValue
...
Next_si_0014 ==
  (\A p$36 \in { "c1", "c2" }:
    ...
    /\ (\E p$37 \in { "c1", "c2" }:
      step[p$37] = "PREVOTE"
        /\ (\E MyEvidence$3 \in SUBSET (msgsPrevote[round[p$37]]):
          LET (*
            @type: (() => Set(Str));
          *)
    ...
    /\ (\E p$39 \in { "c1", "c2" }:
      step[p$39] = "PREVOTE"
        ...
        *)
        PV_si_5 ==
          {
            m$12 \in msgsPrevote[round[p$39]]:
              m$12["id"] = <<"None", (-1), (-1)>>
          }
        IN
