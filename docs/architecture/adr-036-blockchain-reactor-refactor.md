...
executing them against the ABCI application. The current architecture diagram of the blockchain reactor can be found 
here: 
...
messages from/to peers and triggering timeouts. What messages should be sent and timeouts triggered is defined mostly 
...
As explained above, the `Executor` is responsible for sending and receiving messages that are part of `fast-sync` 
...
```
`MessageStatusRequest` is sent periodically to all peers as a request for a peer to provide peer current height. It is 
...
`EventStatusResponse` event is generated from `MessageStatusResponse`.
...
`EventBlockRequest` event is generated upon reception of `MessageBlockRequest` message.  
...
```
`EventBlockResponse` event is generated upon reception of `MessageBlockResponse` message.
...
`PeerStats` data structure keeps of every peer its current height and a list of pending requests for blocks. 

``` go
type PeerStats struct {
  Height             int64
  PendingRequests    List[int64]             // list of outstanding requests sent to this peer
}
```
...
func handleEvent(state ControllerState, event Event) (ControllerState, Message, TimeoutTrigger, Error) {
  ...
  case ModeFastSync:
    ...
    case EventStatusResponse:
      return handleStatusResponse(event, state)
...
``` go
func handleStatusResponse(event EventStatusResponse, state ControllerState) (ControllerState, MessageToSend, TimeoutTrigger, Error) {
  if _, ok := state.PeerMap[event.PeerID]; !ok {
    peerStats = PeerStats{ -1, []int{}, 0 }
  } else {
  ...
  if peerStats.PendingRequestsNum < MAX_NUMBER_OF_PENDING_REQUESTS_PER_PEER {
    msgs = createBlockRequestMessages(state, event.PeerID, peerStats.Height)
    // msgs is empty if no request for block can be made to a peer at this point in time
...
func createBlockRequestMessage(state ControllerState, peerID ID, peerHeight int64) BlockRequestMessage {
  ...
  if blockHeight > -1 { msg = BlockRequestMessage{ blockHeight, peerID } }
  return msg
}
``` 
