...
const (
    ...
    ResponseTimeoutTrigger
    TerminationTimeoutTrigger 
)

type ResponseTimeoutTrigger struct {
    PeerID ID,
    Height int64,
    Duration time.Duration
}

type TerminationTimeoutTrigger struct { 
    Height int64,
    Duration time.Duration 
}
...
and `ConsensusMode` in which it executes Tendermint consensus protocol. We assume that "fast sync" mode terminates once the Controller switch to `ConsensusMode`.  
        ...
        case FastSyncMode: 
            switch event := event.(type) {
                ...
                case StatusReport:
                    ...
                    } else { peerStats = state.PeerMap[event.PeerID] }
                ...
                case RemovePeer:
                    if state.PeerMap[event.PeerID] exists {
                        ...
                        if state.PeerMap.isEmpty() or state.PendingRequestsNum == 0 {
                            timeout = TerminationTimeoutTrigger { state.Height, TerminationTimeout }
                        }      
                ...
                case TerminationTimeout:
                    if state.Height == event.Height and state.PendingRequestsNum == 0 {
