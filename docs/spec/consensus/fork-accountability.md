...
The agreement property says that for a given height, any to correct validators that decide on a block for that height decide on the same block. That the block was indeed generated by the blockchain, can be verified starting from a trusted (genesis) block, by checking that all subsequent blocks are properly signed.
