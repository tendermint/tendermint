...
#### R4: Starvation Prevention ;)
In a churning network with many validator set changes, minimal guarantees for chance in future election should be offered, e.g. for stable lower power validators.


## Proposer Selection Specification
__[R1]__ The proposer algorithm is deterministic giving consistent results accross executions with same transactions and validator set modifications. A pseudocode of the alorithm is given below. 
...
The fairness of the proposer selection is ensured by assigning priorties to each validator based on the voting power and updating them at each round. In addition, everytime a validator is elected as proposer its priority is decreased with the total voting power.
...
Let's assume that the last round was R and the proposer priorites were as shown in first row (R end) with their sum being 0. At this point p2 is removed and round 1 of next height runs. At the end of the round (penultimate row) the sum of priorites is -2 (minus the priority of the removed process). 
...
For this reasons, the selection procedure adds another step (see last row) that shifts the current priority values left or right such that the average remains 0.
...
One other issue that needs to be addressed is the following. A validator V with low voting power that has just been elected is moved to the end of the queue. If the validator set is large and/ or with other validators with significaly higher power, validator V will have to wait a lot of rounds to be elected. If V could remove and readd itself to the validator set, it would make a significant (albeit unfair) "jump" ahead in the queue, decreasing the numbers of rounds it has to wait. 

In order to prevent this, when a new validator is added, its initial priority is not set to 0 but to a lower value. This value is determined from the simulation of the above scenario: it is assumed that V had just propsed a block and was at the back of the queue when it was removed and readded. In addition, a penalty factor is applied.
...
With a penalty factor of 1.125, the inital priority for a newly added verifier is:
