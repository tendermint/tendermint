...
func TestVoteExtensionsAreAlwaysSigned(t *testing.T) {
	...
	assert.NoError(t, err, "expected no error signing vote")
	assert.NotNil(t, vpb1.VoteExtensions.Default[0].Signature)

	extSignItem1, err := types.MakeVoteExtensionSignItems(chainID, vpb1, quorumType, quorumHash)
	require.NoError(t, err)
	assert.True(t, pubKey.VerifySignatureDigest(extSignItem1[types.DefaultExtensionType][0].ID, vpb1.VoteExtensions.Default[0].Signature))

	// We duplicate this vote precisely, including its timestamp, but change
	// its extension
	vote2 := vote1.Copy()
	vote2.VoteExtensions = types.VoteExtensions{
		types.DefaultExtensionType: []types.VoteExtension{{Extension: []byte("new extension")}},
	}
	vpb2 := vote2.ToProto()
	...
	require.NoError(t, err)
	assert.True(t, pubKey.VerifySignatureDigest(extSignItem2[types.DefaultExtensionType][0].ID, vpb2.VoteExtensions.Default[0].Signature))
	assert.False(t, pubKey.VerifySignatureDigest(extSignItem1[types.DefaultExtensionType][0].ID, vpb2.VoteExtensions.Default[0].Signature))

	vpb2.BlockSignature = nil
	vpb2.StateSignature = nil
	vpb2.VoteExtensions.Default[0].Signature = nil
	...
	require.NoError(t, err)
	assert.True(t, pubKey.VerifySignatureDigest(extSignItem3[types.DefaultExtensionType][0].ID, vpb2.VoteExtensions.Default[0].Signature))
	assert.False(t, pubKey.VerifySignatureDigest(extSignItem1[types.DefaultExtensionType][0].ID, vpb2.VoteExtensions.Default[0].Signature))
}
