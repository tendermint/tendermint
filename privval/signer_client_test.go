...
func TestSignerVote(t *testing.T) {
	for _, tc := range getSignerTestCases(t) {
		ts := time.Now()
		want := &types.Vote{Timestamp: ts, Type: types.SIGNED_MSG_TYPE_PRECOMMIT}
		have := &types.Vote{Timestamp: ts, Type: types.SIGNED_MSG_TYPE_PRECOMMIT}
...
func TestSignerSignVoteErrors(t *testing.T) {
	for _, tc := range getSignerTestCases(t) {
		ts := time.Now()
		vote := &types.Vote{Timestamp: ts, Type: types.SIGNED_MSG_TYPE_PRECOMMIT}
