...
func TestConditions(t *testing.T) {
	...
	testCases := []struct {
		...
		{s: "tm.events.type='NewBlock'", conditions: []query.Condition{{Event: "tm.events.type", Op: query.OpEqual, Operand: "NewBlock"}}},
		{s: "tx.gas > 7 AND tx.gas < 9", conditions: []query.Condition{{Event: "tx.gas", Op: query.OpGreater, Operand: int64(7)}, {Event: "tx.gas", Op: query.OpLess, Operand: int64(9)}}},
		{s: "tx.time >= TIME 2013-05-03T14:45:00Z", conditions: []query.Condition{{Event: "tx.time", Op: query.OpGreaterEqual, Operand: txTime}}},
	}
