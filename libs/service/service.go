...
func (bs *BaseService) Stop() error {
	if atomic.CompareAndSwapUint32(&bs.stopped, 0, 1) {
		if atomic.LoadUint32(&bs.started) == 0 {
			bs.Logger.Error(fmt.Sprintf("Not stopping %v service -- have not been started yet", bs.name),
				"impl", bs.impl)
			// revert flag
	...
	bs.Logger.Debug(fmt.Sprintf("Stopping %v service (ignoring: already stopped)", bs.name), "impl", bs.impl)
	return ErrAlreadyStopped
}
