...
func makeUPNPListener(intPort int, extPort int, logger log.Logger) (NAT, net.Listener, net.IP, error) {
	...
	logger.Info("make upnp listener", log.LazySprintf("ourIP: %v", nat.(*upnpNAT).ourIP))

	ext, err := nat.GetExternalAddress()
	if err != nil {
		return nat, nil, nil, fmt.Errorf("external address error: %v", err)
	}
	logger.Info("make upnp listener", log.LazySprintf("External address: %v", ext))
	...
	logger.Info("make upnp listener", log.LazySprintf("Port mapping mapped: %v", port))
...
func testHairpin(listener net.Listener, extAddr string, logger log.Logger) (supportsHairpin bool) {
	...
	go func() {
		...
		if err != nil {
			logger.Info("test hair pin", log.LazySprintf("Listener.Accept() error: %v", err))
			return
		}
		logger.Info("test hair pin",
			log.LazySprintf("Accepted incoming connection: %v -> %v", inConn.LocalAddr(), inConn.RemoteAddr()))
		buf := make([]byte, 1024)
		n, err := inConn.Read(buf)
		if err != nil {
			logger.Info("test hair pin", log.LazySprintf("Incoming connection read error: %v", err))
			return
		}
		logger.Info("test hair pin", log.LazySprintf("Incoming connection read %v bytes: %X", n, buf))
	...
	if err != nil {
		logger.Info("test hair pin", log.LazySprintf("Outgoing connection dial error: %v", err))
		return
	}

	n, err := outConn.Write([]byte("test data"))
	if err != nil {
		logger.Info("test hair pin", log.LazySprintf("Outgoing connection write error: %v", err))
		return
	}
	logger.Info("test hair pin", log.LazySprintf("Outgoing connection wrote %v bytes", n))
