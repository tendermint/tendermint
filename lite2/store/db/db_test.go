...
func TestLast_FirstSignedHeaderHeight(t *testing.T) {
	...
	err = dbStore.SaveSignedHeaderAndValidatorSet(
		&types.SignedHeader{Header: &types.Header{Height: 1, ProposerAddress: tmrand.Bytes(20)}}, vals)
	require.NoError(t, err)
...
func Test_SignedHeaderBefore(t *testing.T) {
	...
	err := dbStore.SaveSignedHeaderAndValidatorSet(
		&types.SignedHeader{Header: &types.Header{Height: 2, ProposerAddress: tmrand.Bytes(20)}}, valSet)
	require.NoError(t, err)
...
func Test_Prune(t *testing.T) {
	...
	for i := 1; i <= 10; i++ {
		err = dbStore.SaveSignedHeaderAndValidatorSet(
			&types.SignedHeader{Header: &types.Header{Height: int64(i), ProposerAddress: tmrand.Bytes(20)}}, valSet)
		require.NoError(t, err)
	}
	...
	for i := 1; i <= 100; i++ {
		...
		go func(i int64) {
			...
			dbStore.SaveSignedHeaderAndValidatorSet(
				&types.SignedHeader{Header: &types.Header{Height: i, ProposerAddress: tmrand.Bytes(20)}}, vals)
