...
func TestVerifyAdjacentHeaders(t *testing.T) {
	...
	testCases := []struct {
		...
		7: {
			...
			3 * time.Hour,
			bTime.Add(2 * time.Hour),
			ErrInvalidHeader{Reason: types.ErrNotEnoughVotingPowerSigned{Got: 50, Needed: 93}.Error()},
			"",
		},
...
func TestVerifyNonAdjacentHeaders(t *testing.T) {
	...
	testCases := []struct {
		...
		2: {
			...
			3 * time.Hour,
			bTime.Add(2 * time.Hour),
			ErrInvalidHeader{types.ErrNotEnoughVotingPowerSigned{Got: 50, Needed: 93}.Error()},
			"",
		},
