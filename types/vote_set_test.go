...
func TestAddVote(t *testing.T) {
	...
	voteSet, _, privValidators := randVoteSet(height, round, SIGNED_MSG_TYPE_PREVOTE_TYPE, 10, 1)
...
func Test2_3MajorityRedux(t *testing.T) {
	height, round := int64(1), 0
	voteSet, _, privValidators := randVoteSet(height, round, SIGNED_MSG_TYPE_PREVOTE_TYPE, 100, 1)

	blockHash := crypto.CRandBytes(32)
	blockPartsTotal := 123
	blockPartsHeader := PartSetHeader{int64(blockPartsTotal), crypto.CRandBytes(32)}
	...
	// 69th validator voted for different BlockParts Total
	{
		...
		blockPartsHeader := PartSetHeader{int64(blockPartsTotal + 1), blockPartsHeader.Hash}
...
func TestBadVotes(t *testing.T) {
	...
	// val3 votes of another type.
	{
		...
		added, err := signAddVote(privValidators[3], withType(vote, byte(SIGNED_MSG_TYPE_PRECOMMIT_TYPE)), voteSet)
		if added || err == nil {
...
func TestConflicts(t *testing.T) {
	height, round := int64(1), 0
	voteSet, _, privValidators := randVoteSet(height, round, SIGNED_MSG_TYPE_PREVOTE_TYPE, 4, 1)
...
func TestMakeCommit(t *testing.T) {
	height, round := int64(1), 0
	voteSet, _, privValidators := randVoteSet(height, round, SIGNED_MSG_TYPE_PRECOMMIT_TYPE, 10, 1)
