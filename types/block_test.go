...
func TestBlockAddEvidence(t *testing.T) {
	...
	voteSet, valSet, vals := randVoteSet(h-1, 1, Msg_type_precommit, 10, 1)
...
func TestCommit(t *testing.T) {
	lastID := makeBlockIDRandom()
	h := int64(3)
	voteSet, _, vals := randVoteSet(h-1, 1, Msg_type_precommit, 10, 1)
	commit, err := MakeCommit(lastID, h-1, 1, voteSet, vals)
	require.NoError(t, err)

	assert.Equal(t, h-1, commit.Height)
	assert.Equal(t, 1, commit.Round)
	assert.Equal(t, Msg_type_precommit, SignedMsgType(commit.Type()))
	if commit.Size() <= 0 {
...
func TestCommitToVoteSetWithVotesForNilBlock(t *testing.T) {
	...
	for _, tc := range testCases {
		voteSet, valSet, vals := randVoteSet(height-1, round, Msg_type_precommit, tc.numValidators, 1)
