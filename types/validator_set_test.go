...
func TestProposerSelection3(t *testing.T) {
	...
	for ; i < 10000; i++ {
		...
		// times is usually 1
		times := 1
		mod := (tmrand.NewRand().Int() % 5) + 1
		if tmrand.NewRand().Int()%mod > 0 {
			// sometimes its up to 5
			times = (tmrand.NewRand().Int() % 4) + 1
		}
...
func randPubKey() crypto.PubKey {
	var pubKey [32]byte
	copy(pubKey[:], tmrand.NewRand().Bytes(32))
...
func randValidator(totalVotingPower int64) *Validator {
	...
	val := NewValidator(randPubKey(), int64(tmrand.NewRand().Uint64()%uint64((MaxTotalVotingPower-totalVotingPower))))
	val.ProposerPriority = tmrand.NewRand().Int64() % (MaxTotalVotingPower - totalVotingPower)
	return val
}
...
func permutation(valList []testVal) []testVal {
	...
	perm := tmrand.NewRand().Perm(len(valList))
	for i, v := range perm {
...
func randTestVSetCfg(t *testing.T, nBase, nAddMax int) testVSetCfg {
	...
	nOld = int(tmrand.NewRand().Uint()%uint(nBase)) + 1
	if nBase-nOld > 0 {
		nDel = int(tmrand.NewRand().Uint() % uint(nBase-nOld))
	}
	nChanged = nBase - nOld - nDel

	if nAddMax > 0 {
		nAdd = tmrand.NewRand().Int()%nAddMax + 1
	}
	...
	for i := 0; i < nBase; i++ {
		cfg.startVals[i] = testVal{fmt.Sprintf("v%d", i), int64(tmrand.NewRand().Uint()%maxPower + 1)}
		if i < nOld {
			cfg.expectedVals[i] = cfg.startVals[i]
		}
		if i >= nOld && i < nOld+nChanged {
			cfg.updatedVals[i-nOld] = testVal{fmt.Sprintf("v%d", i), int64(tmrand.NewRand().Uint()%maxPower + 1)}
	...
	for i := nBase; i < nBase+nAdd; i++ {
		cfg.addedVals[i-nBase] = testVal{fmt.Sprintf("v%d", i), int64(tmrand.NewRand().Uint()%maxPower + 1)}
...
func verifyValSetUpdatePriorityOrder(t *testing.T, valSet *ValidatorSet, cfg testVSetCfg, nMaxElections int) {
	...
	valSet.IncrementProposerPriority(tmrand.NewRand().Int()%nMaxElections + 1)
