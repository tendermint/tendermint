...
func (tm2pb) Validator(val *Validator) abcix.Validator {
...
func (tm2pb) ValidatorUpdate(val *Validator) abcix.ValidatorUpdate {
	pk, err := cryptoenc.PubKeyToProto(val.PubKey)
	if err != nil {
		panic(err)
	}
	return abcix.ValidatorUpdate{
		PubKey: pk,
		Power:  val.VotingPower,
	}
}
...
func (tm2pb) ValidatorUpdates(vals *ValidatorSet) []abcix.ValidatorUpdate {
	validators := make([]abcix.ValidatorUpdate, vals.Size())
...
func (tm2pb) ConsensusParams(params *tmproto.ConsensusParams) *abcix.ConsensusParams {
	return &abcix.ConsensusParams{
...
// XXX: panics on nil or unknown pubkey type
func (tm2pb) Evidence(ev Evidence, valSet *ValidatorSet, evTime time.Time) abcix.Evidence {
	addr := ev.Address()
...
// XXX: panics on nil or unknown pubkey type
func (tm2pb) NewValidatorUpdate(pubkey crypto.PubKey, power int64) abcix.ValidatorUpdate {
...
func (pb2tm) ValidatorUpdates(vals []abcix.ValidatorUpdate) ([]*Validator, error) {
