...
var fileDescriptor_57e39eefdaf7ae96 = []byte{
	...
	0xf0, 0x04, 0x41, 0x38, 0x4e, 0x59, 0x17, 0x1e, 0xca, 0x31, 0xdc, 0x78, 0x28, 0xc7, 0xf0, 0xe1,
	...
	0x33, 0x4b, 0x32, 0x4a, 0x93, 0xf4, 0x92, 0xf3, 0x73, 0xf5, 0x11, 0xde, 0x41, 0x66, 0x62, 0x09,
	0x85, 0x24, 0x36, 0xb0, 0xa0, 0x31, 0x20, 0x00, 0x00, 0xff, 0xff, 0x75, 0xa7, 0x51, 0x0d, 0x23,
	0x01, 0x00, 0x00,
}

func (this *SimpleProof) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*SimpleProof)
	if !ok {
		that2, ok := that.(SimpleProof)
		if ok {
			that1 = &that2
		} else {
			return false
	...
	if !bytes.Equal(this.LeafHash, that1.LeafHash) {
		return false
	}
	if len(this.Aunts) != len(that1.Aunts) {
		return false
	}
	for i := range this.Aunts {
		if !bytes.Equal(this.Aunts[i], that1.Aunts[i]) {
			return false
		}
	}
	return true
}
func (this *SimpleProof) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 8)
	s = append(s, "&merkle.SimpleProof{")
	s = append(s, "Total: "+fmt.Sprintf("%#v", this.Total)+",\n")
	s = append(s, "Index: "+fmt.Sprintf("%#v", this.Index)+",\n")
	s = append(s, "LeafHash: "+fmt.Sprintf("%#v", this.LeafHash)+",\n")
	s = append(s, "Aunts: "+fmt.Sprintf("%#v", this.Aunts)+",\n")
	s = append(s, "}")
...
func (this *SimpleProof) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&SimpleProof{`,
		`Total:` + fmt.Sprintf("%v", this.Total) + `,`,
		`Index:` + fmt.Sprintf("%v", this.Index) + `,`,
		`LeafHash:` + fmt.Sprintf("%v", this.LeafHash) + `,`,
		`Aunts:` + fmt.Sprintf("%v", this.Aunts) + `,`,
		`}`,
	}, "")
	return s
}
