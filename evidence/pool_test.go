...
func TestLightClientAttackEvidenceLifecycle(t *testing.T) {
	...
	evData := types.EvidenceData{Evidence: pendingEv}
	hash1 := evData.Hash()

	pb, err := evData.ToProto()
	require.NoError(t, err)

	evDataAfter := types.EvidenceData{}
	evDataAfter.FromProto(pb)

	require.Equal(t, hash1, evDataAfter.Hash())
}
