...
func (evpool *Pool) AddPOLC(polc types.ProofOfLockChange) error {
	...
	polcBytes, err := proto.Marshal(pb)
	if err != nil {
		panic(err)
	}
...
func (evpool *Pool) addPendingEvidence(evidence types.Evidence) error {
	evipb, err := types.EvidenceToProto(evidence)
	if err != nil {
		return fmt.Errorf("error on evidence to proto: %w", err)
	}
	evBytes, err := proto.Marshal(evipb)
	if err != nil {
...
func (evpool *Pool) removeExpiredPendingEvidence() {
	...
	for ; iter.Valid(); iter.Next() {
		...
		err := proto.Unmarshal(evBytes, &pb)
		if err != nil {
			panic(err)
		}
...
func (evpool *Pool) pruneExpiredPOLC() {
	...
	for ; iter.Valid(); iter.Next() {
		...
		var pb tmproto.ProofOfLockChange
		err := proto.Unmarshal(proofBytes, &pb)
		if err != nil {
			panic(err)
		}

		proof, err := types.ProofOfLockChangeFromProto(&pb)
		if err != nil {
			evpool.logger.Error("Unable to unmarshal POLC", "err", err)
			continue
		}
