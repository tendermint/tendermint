...
func (rt *Routine) start() {
	rt.logger.Info("routine start", "msg", log.LazySprintf("%s: run", rt.name))
	...
	for {
		...
		rt.logger.Debug("routine start", "msg", log.LazySprintf("%s: produced %T %+v", rt.name, oEvent, oEvent))
...
func (rt *Routine) send(event Event) bool {
	rt.logger.Debug("routine send", "msg", log.LazySprintf("%s: received %T %+v", rt.name, event, event))
	if !rt.isRunning() {
		return false
	}
...
func (rt *Routine) stop() {
	...
	rt.logger.Info("routine stop", "msg", log.LazySprintf("%s: stop", rt.name))
